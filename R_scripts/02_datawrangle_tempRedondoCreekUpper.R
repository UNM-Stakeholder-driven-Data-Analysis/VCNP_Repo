...
LJWB_file_list = list.files(path="C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/raw data/Valles_Flume_CSVs_Corrected/Redondo Creek Upper/", 
                            recursive=F, 
                            full.names=TRUE)
...
# View(LJWB_HOBO_list[[35]]) #need to correct date from 1/9/1930 to 11/2/17
# View(LJWB_HOBO_list[[36]]) #need to correct date from 1/9/2030 to 10/16/18
...
LJWB_HOBO_list[[36]]$Date.Time..GMT.06.00.1 <- format(strptime(LJWB_HOBO_list[[36]]$Date.Time..GMT.06.00, format="%m/%d/%y %I:%M:%S %p"), format="%H:%M:%S")
LJWB_HOBO_list[[36]]$Date.Time..GMT.06.00 <- as.Date(LJWB_HOBO_list[[36]]$Date.Time..GMT.06.00, format="%m/%d/%y")

LJWB_HOBO_list[[36]] <- LJWB_HOBO_list[[36]] %>% relocate(Date.Time..GMT.06.00.1, .after=Date.Time..GMT.06.00)
...
View(LJWB_HOBO_list_2[[36]])

#### format date ####
for(i in 1:length(LJWB_HOBO_list_2)){
  LJWB_HOBO_list_2[[i]]$date =  as.Date(LJWB_HOBO_list_2[[i]][,1], format = "%m/%d/%Y")
}
# I will correct dates in df 24
View(LJWB_HOBO_list_2[[24]])
range(LJWB_HOBO_list_2[[24]]$date)
...
# apply correction
LJWB_HOBO_list_2[[24]]$date = LJWB_HOBO_list_2[[24]]$date + diff_date
# check new range
range(LJWB_HOBO_list_2[[24]]$date)

# Now do the same for 35 - from 1/9/1930 to 11/2/17
View(LJWB_HOBO_list_2[[35]])
range(LJWB_HOBO_list_2[[35]]$date)
diff_date = as.Date("2017-11-02") - as.Date("1930-01-09")
LJWB_HOBO_list_2[[35]]$date = LJWB_HOBO_list_2[[35]]$date + diff_date
range(LJWB_HOBO_list_2[[35]]$date)
# Now do the same for 36 - from 1/9/2030 to 10/16/18
View(LJWB_HOBO_list_2[[36]])
range(LJWB_HOBO_list_2[[36]]$date)
diff_date = as.Date("2030-01-09") - as.Date("2018-10-16")
LJWB_HOBO_list_2[[36]]$date = LJWB_HOBO_list_2[[36]]$date - diff_date
range(LJWB_HOBO_list_2[[36]]$date)
...
LJWB_HOBO_hrly_ts_export$siteID = "RedondoCreekUpper"

#export as csv
write.csv(LJWB_HOBO_hrly_ts_export,  "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/alldata/RedondoCreekUpper_temp.csv")
...
dat_monthly =
  ...
  summarise(Value.mn = mean(temp_C_c, na.rm = T), 
            ...
            Value.n = n())
dat_monthly$siteID = "RedondoCreekUpper"
...
#export as csv
write.csv(dat_monthly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/monthly/RedondoCreekUpper_temp_monthly.csv")
 ...
 #get yearly averages
          dat_yearly =
            ...
            summarise(Value.mn = mean(temp_C_c, na.rm = T), 
                      Value.sd = sd(temp_C_c, na.rm = T), 
                      Value.n = n())
          dat_yearly$siteID = "RedondoCreekUpper"
          #export as csv
          write.csv(dat_yearly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/yearly/RedondoCreekUpper_temp_yr.csv")
# plot 
...
ggsave(path="QAQC plots", filename="RedondoCreekUpper.png", width=6.4, height=5.5, units="in")
