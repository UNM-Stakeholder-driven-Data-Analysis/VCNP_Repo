...
ggplot(data=LJWB_HOBO_hrly_ts, aes(x=datetime_NM, y =temp_C))+
  ...
  theme_bw() +
  scale_x_datetime(limits=c(as.POSIXct('2013-06-22 00:00:00', tz="America/Denver"), as.POSIXct('2013-06-30 00:00:00', tz="America/Denver")), date_breaks="1 days") +
  scale_y_continuous(limits=c(0,30))

#need to delete summer data where logger wasn't stopped but appears to be removed from water
LJWB_HOBO_hrly_ts$date <- as.Date(LJWB_HOBO_hrly_ts$datetime_NM)
LJWB_HOBO_hrly_ts_1st <- filter(LJWB_HOBO_hrly_ts, date < as.Date("2017-06-30"))
LJWB_HOBO_hrly_ts_2nd <- filter(LJWB_HOBO_hrly_ts, date>as.Date("2017-07-25"))
LJWB_HOBO_hrly_ts<-rbind(LJWB_HOBO_hrly_ts_1st, LJWB_HOBO_hrly_ts_2nd)
LJWB_HOBO_hrly_ts_3rd <- filter(LJWB_HOBO_hrly_ts, date < as.Date("2013-06-23"))
LJWB_HOBO_hrly_ts_4th <- filter(LJWB_HOBO_hrly_ts, date>as.Date("2013-06-27"))
...
#QAQC temp threshold to identify in-air points is set here to be x1.1 the upper (75%) quantile of temp data on each date. Can edit by changing either quantile probability or what it is divided by. Can also add a lower threshold, although this would not be indicative of in-air in summer data.
...
LJWB_HOBO_hrly_ts_export$siteID = "RioSanAntonioToledo"

#export as csv
write.csv(LJWB_HOBO_hrly_ts_export,  "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/alldata/RioSanAntonioToledo_temp.csv")
...
dat_monthly =
  ...
  summarise(Value.mn = mean(temp_C_c, na.rm = T), 
            ...
            Value.n = n())
dat_monthly$siteID = "RioSanAntonioToledo"
...
#export as csv
write.csv(dat_monthly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/monthly/RioSanAntonioToledo_temp_monthly.csv")
...
#get yearly averages
          dat_yearly =
            ...
            summarise(Value.mn = mean(temp_C_c, na.rm = T), 
                      Value.sd = sd(temp_C_c, na.rm = T), 
                      Value.n = n())
          dat_yearly$siteID = "RioSanAntonioToledo"
          #export as csv
          write.csv(dat_yearly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/yearly/RioSanAntonioToledo_temp_yr.csv")
...
ggsave(path="QAQC plots", filename="RioSanAntonioToledo.png", width=6.4, height=5.5, units="in")
