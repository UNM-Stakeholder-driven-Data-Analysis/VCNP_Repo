...
# faceted by year:
# add year and doy
LJWB_HOBO_hrly_ts_1$yr = lubridate::year(LJWB_HOBO_hrly_ts_1$datetime_NM)
LJWB_HOBO_hrly_ts_1$day = lubridate::yday(LJWB_HOBO_hrly_ts_1$datetime_NM)
# add month
LJWB_HOBO_hrly_ts_1$mo = lubridate::month(LJWB_HOBO_hrly_ts_1$datetime_NM)
# add season (i'm estimating on season here - could be more accutate using dates)
LJWB_HOBO_hrly_ts_1$season = ifelse(LJWB_HOBO_hrly_ts_1$mo %in% c(11,12,1,2,3) , "Winter", "Summer")

#export as csv
write.csv(LJWB_HOBO_hrly_ts_1, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/LaJaraTemp.csv")
...
# plot
ggplot(data=LJWB_HOBO_hrly_ts_1, aes(x=datetime_NM, y=temp_C, color=season))+
  geom_point() + 
  theme(legend.title = element_blank()) +
  theme_bw()
...
# decide if/how the data needs cleaning. Right now I'm not sure that the negative values are 
#inaccurate, since they occur mostly in winter. However, I remember David talking about 
#them going wonky when they get encased in ice. So, plot this up and discuss with him. 