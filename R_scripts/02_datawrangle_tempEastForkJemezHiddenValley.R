...
# View(LJWB_HOBO_list[[52]]) #correct F to C
...
#convert_F_to_C
View(LJWB_HOBO_list_2[[52]])
#rename_F_data_col
colnames(LJWB_HOBO_list_2[[52]])[3] <- "Original_F"
# calculate correction
LJWB_HOBO_list_2[[52]]$Temp_C = (LJWB_HOBO_list_2[[52]]$Original_F-32)*(5/9)
...
ggplot(data=LJWB_HOBO_hrly_ts, aes(x=datetime_NM, y =Temp_C_c))+
  ...
  theme_bw() +
  scale_x_datetime(limits=c(as.POSIXct('2008-04-15 00:00:00', tz="America/Denver"), as.POSIXct('2008-04-26 00:00:00', tz="America/Denver")), date_breaks="2 days") + 
  scale_y_continuous(limits=c(0,30))

#need to delete winter data where logger was out of water prior to initial install
LJWB_HOBO_hrly_ts$date = lubridate::date(LJWB_HOBO_hrly_ts$datetime_NM)
LJWB_HOBO_hrly_ts <- filter(LJWB_HOBO_hrly_ts, date > as.Date("2008-04-24"))
...
LJWB_HOBO_hrly_ts_export$siteID = "EastForkJemezHiddenValley"

#export as csv
write.csv(LJWB_HOBO_hrly_ts_export, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/alldata/EastForkJemezHiddenValley_temp.csv")
...
dat_monthly =
  ...
  summarise(Value.mn = mean(temp_C_c, na.rm = T), 
            ...
            Value.n = n())
dat_monthly$siteID = "EastForkJemezHiddenValley"
...
#export as csv
write.csv(dat_monthly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/monthly/EastForkJemezHiddenValley_temp_monthly.csv")
...
dat_yearly =
  ...
  summarise(Value.mn = mean(temp_C_c, na.rm = T), 
            Value.sd = sd(temp_C_c, na.rm = T), 
            Value.n = n())
dat_yearly$siteID = "EastForkJemezHiddenValley"
#export as csv
write.csv(dat_yearly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/yearly/EastForkJemezHiddenValley_temp_yr.csv")

#monthly data:
ggplot(data=dat_monthly, aes(x=date, y=Value.mn))+
  geom_point() + geom_path()+
  theme(legend.title = element_blank()) +
  theme_bw()+
...
ggsave(path="QAQC plots", filename="EastForkJemezHiddenValley.png", width=6.4, height=5.5, units="in")
