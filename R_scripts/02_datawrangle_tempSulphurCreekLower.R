...
LJWB_file_list = list.files(path="C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/raw data/Valles_Flume_CSVs_Corrected/Sulphur Creek Lower/", 
                            recursive=F, 
                            full.names=TRUE)
...
# View(LJWB_HOBO_list[[23]]) #need to change date from 1/10/1937 to 7/25/12
...
# View(LJWB_HOBO_list[[47]]) #need to split date/time col
# View(LJWB_HOBO_list[[48]]) #need to split date/time col

# These look like they are structured the same. Except list 1 and 2 need first two columns split

LJWB_HOBO_list[[47]]$Date.Time..GMT.06.00.1 <- format(strptime(LJWB_HOBO_list[[47]]$Date.Time..GMT.06.00, format="%m/%d/%y %I:%M:%S %p"), format="%H:%M:%S")
LJWB_HOBO_list[[47]]$Date.Time..GMT.06.00 <- as.Date(LJWB_HOBO_list[[47]]$Date.Time..GMT.06.00, format="%m/%d/%y")
LJWB_HOBO_list[[47]] <- LJWB_HOBO_list[[47]] %>% relocate(Date.Time..GMT.06.00.1, .after=Date.Time..GMT.06.00)

LJWB_HOBO_list[[48]]$Date.Time..GMT.06.00.1 <- format(strptime(LJWB_HOBO_list[[48]]$Date.Time..GMT.06.00, format="%m/%d/%y %I:%M:%S %p"), format="%H:%M:%S")
LJWB_HOBO_list[[48]]$Date.Time..GMT.06.00 <- as.Date(LJWB_HOBO_list[[48]]$Date.Time..GMT.06.00, format="%m/%d/%y")
LJWB_HOBO_list[[48]] <- LJWB_HOBO_list[[48]] %>% relocate(Date.Time..GMT.06.00.1, .after=Date.Time..GMT.06.00)
...
View(LJWB_HOBO_list_2[[23]])
range(LJWB_HOBO_list_2[[23]]$date)
...
# apply correction
LJWB_HOBO_list_2[[23]]$date = LJWB_HOBO_list_2[[23]]$date + diff_date
# check new range
range(LJWB_HOBO_list_2[[23]]$date)
...
LJWB_HOBO_hrly_ts_export$siteID = "SulphurCreekLower"

#export as csv
write.csv(LJWB_HOBO_hrly_ts_export, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/alldata/SulphurCreekLower_temp.csv")
...
dat_monthly =
  ...
  summarise(Value.mn = mean(temp_C_c, na.rm = T), 
            ...
            Value.n = n())
dat_monthly$siteID = "SulphurCreekLower"
...
#export as csv
write.csv(dat_monthly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/monthly/SulphurCreekLower_temp_monthly.csv")
...
#get yearly averages
          dat_yearly =
            ...
            summarise(Value.mn = mean(temp_C_c, na.rm = T), 
                      Value.sd = sd(temp_C_c, na.rm = T), 
                      Value.n = n())
          dat_yearly$siteID = "SulphurCreekLower"
 #export as csv
          write.csv(dat_yearly, "C:/Users/Brionna/OneDrive - University of New Mexico/Classes/EPS545_BIO502/VCNP/VCNP_Repo/processed data/yearly/SulphurCreekLower_temp_yr.csv")
...
ggsave(path="QAQC plots", filename="SulphurCreekLower.png", width=6.4, height=5.5, units="in")
